...
extension (req: Request)
  ...
  def hattrickPeriod(): IO[BadRequestError, HattrickPeriod] = {
    ...
    period match {
      case "round" => ZIO.succeed(service.Round)
      case "season" => ZIO.succeed(service.Season)
      case "weeks" => req.intParam("weeksNumber").map(service.Weeks(_))
