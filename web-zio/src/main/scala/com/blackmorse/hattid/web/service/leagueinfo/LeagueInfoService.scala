...
import com.blackmorse.hattid.web.databases.requests.matchdetails.HistoryInfoRequest
import com.blackmorse.hattid.web.models.clickhouse.HistoryInfo
import com.blackmorse.hattid.web.models.web.league.LeagueTime
import com.blackmorse.hattid.web.service.leagueinfo.LeagueInfoServiceZIO.{DivisionLevel, LeagueId, Round, Season}
...
class LeagueInfoServiceZIO(private val leagueInfoMap: ConcurrentMap[LeagueId, LeagueInfoZIO]) {
  ...
  def addAnotherRound(leagueId: LeagueId, 
                      season: Season, 
                      round: Round, 
                      roundInfos: List[HistoryInfo]): ZIO[CHPPServices, HattidInternalError, Unit] = {
    for {
