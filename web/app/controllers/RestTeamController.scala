...
class RestTeamController @Inject() (val controllerComponents: ControllerComponents,
  ...
  def getTeamData(teamId: Long): Action[JsValue] = asyncZio {
  ...
  def playerGoalGames(teamId: Long, restStatisticsParameters: RestStatisticsParameters, playersParameters: PlayersParameters): Action[JsValue] =
  ...
  def playerTsiSalary(teamId: Long, restStatisticsParameters: RestStatisticsParameters, playersParameters: PlayersParameters): Action[JsValue] =
    playersRequest(PlayerSalaryTSIRequest, teamId, restStatisticsParameters, playersParameters)

  def playerRatings(teamId: Long, restStatisticsParameters: RestStatisticsParameters, playersParameters: PlayersParameters): Action[JsValue] =
    playersRequest(PlayerRatingsRequest, teamId, restStatisticsParameters, playersParameters)

  def playerInjuries(teamId: Long, restStatisticsParameters: RestStatisticsParameters): Action[JsValue] =
    stats(PlayerInjuryRequest, teamId, restStatisticsParameters)

  def topMatches(teamId: Long, restStatisticsParameters: RestStatisticsParameters): Action[JsValue] =
    stats(MatchTopHatstatsRequest, teamId, restStatisticsParameters)

  def surprisingMatches(teamId: Long, restStatisticsParameters: RestStatisticsParameters): Action[JsValue] =
    stats(MatchSurprisingRequest, teamId, restStatisticsParameters)

  def matchSpectators(teamId: Long, restStatisticsParameters: RestStatisticsParameters): Action[JsValue] =
    stats(MatchSpectatorsRequest, teamId, restStatisticsParameters)

  def teamRankingsRange(teamId: Long, fromSeason: Int, toSeason: Int): Action[JsValue] = asyncZio {
    for {
      team <- chppService.getTeamById(teamId)
  ...
  def teamRankings(teamId: Long, season: Option[Int]): Action[JsValue] = asyncZio {
    for {
      team <- chppService.getTeamById(teamId)
  ...
  def promotions(teamId: Long): Action[JsValue] = asyncZio {
    for {
      team <- chppService.getTeamById(teamId)
  ...
  def teamMatches(teamId: Long, season: Int): Action[JsValue] = asyncZio {
    for {
      team <- chppService.getTeamById(teamId)
      res <- chppService.getDivisionLevelAndLeagueUnit(team, season)
      (divisionLevel, leagueUnitId) = res
