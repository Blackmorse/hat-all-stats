...
class OverviewClickhouseDAO @Inject()(
  ...
  def numberOfTeams(round: Int, season: Int) =
    stats("SELECT count() AS count FROM hattrick.match_details __where__",
      round, season, CountsModel.mapper.single)

  def overviewPlayerState(round: Int, season: Int) =
    stats("SELECT count() as count, countIf(injury_level > 0) AS injuried, sum(goals) as goals, " +
      ...
      round, season, OverviewPlayerStats.mapper.single)

  def topSalaryTeams(round: Int, season: Int) =
    stats("""SELECT any(season) as team_season, any(round) as team_round, league_id, league_unit_id, league_unit_name, team_name, team_id, sum(salary) AS value
                 ...
                 |ORDER BY value DESC
                 |LIMIT 5
                 |""".stripMargin, round, season, TeamOverviewModel.mapper.*)

  def topHatstatsTeams(round: Int, season: Int) =
  ...
  def topHatstatsMatches(round: Int, season: Int) =
    stats("""SELECT league_id, league_unit_id, league_unit_name, team_id, team_name,
                 ...
                 |""".stripMargin, round, season, MatchOverviewModel.mapper.*)

  def topRandomMatches(round: Int, season: Int) =
  ...
  def avgMatchDetails(round: Int, season: Int) =
    stats("""SELECT
            ...
            |FROM hattrick.match_details
            |__where__
            |""".stripMargin, round, season, AvgMatchDetailsModel.mapper.single)

  def avgTeamPlayers(round: Int, season: Int) =
    stats("""SELECT
            ...
            |    GROUP BY team_id
            |)
            |""".stripMargin, round, season, AvgTeamPlayersStats.mapper.single)

  def formations(round: Int, season: Int) =
    stats("""SELECT formation, count() AS count
            |FROM hattrick.match_details __where__
            |GROUP BY formation  ORDER BY count DESC
            |LIMIT 6""".stripMargin, round, season, FormationsModel.mapper.*)

  def topSalaryPlayers(round: Int, season: Int) =
    stats("""SELECT season, round, league_id, league_unit_id, league_unit_name, team_id,
            ...
            |FROM hattrick.player_stats __where
            |ORDER BY value DESC LIMIT 5
            |""".stripMargin, round, season, PlayerOverviewModel.mapper.*)

  def topRatingPlayers(round: Int, season: Int) =
    stats("""SELECT season, round, league_id, league_unit_id, league_unit_name, team_id,
  ...
  private def stats[T](sql: String, round: Int, season: Int, parser: ResultSetParser[T]): T = db.withConnection{ implicit connection =>
    SqlBuilder(sql)
        .round(round)
        .season(season)
        .build
