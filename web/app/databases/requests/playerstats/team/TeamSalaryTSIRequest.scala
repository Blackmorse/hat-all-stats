...
object TeamSalaryTSIRequest extends ClickhouseRequest[TeamSalaryTSI] {
  ...
  def execute(orderingKeyPath: OrderingKeyPath,
    ...
    restClickhouseDAO.execute(simpleSql(orderingKeyPath, parameters, playedInLastMatch, excludeZeroTsi), rowParser)
  }
  
  def executeZIO(orderingKeyPath: OrderingKeyPath,
                 parameters: RestStatisticsParameters,
                 playedInLastMatch: Boolean,
                 excludeZeroTsi: Boolean)(implicit restClickhouseDAO: RestClickhouseDAO): IO[HattidError, List[TeamSalaryTSI]] = {
    if (!sortingColumns.contains(parameters.sortBy)) {
